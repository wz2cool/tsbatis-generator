...
export class MysqlService {
    private numberTypes = ["TINYINT", "SMALLINT", "MEDIUMINT", "INT", "INTEGER",
        "BIGINT", "FLOAT", "DOUBLE", "DECIMAL", "BIT"];
    private stringTypes = ["CHAR", "VARCHAR", "TINYBLOB", "TINYTEXT", "BLOB",
    ...
    public async generateTableEntitiesZipFile(
        uri: string, user: string, pwd: string, database: string, tableNames: string[]): Promise<string> {
        try {
    ...
    public async generateTableEntities(
        uri: string, user: string, pwd: string, database: string, tableNames: string[]): Promise<TextFileInfo[]> {
        try {
    ...
    public async generateTableEntity(
        uri: string, user: string, pwd: string, database: string, tableName: string): Promise<TextFileInfo> {
        try {
            const dbColumInfos = await this.getDbColumnInfos(uri, user, pwd, database, tableName);
